...
func ParseEthernetHeader(flowMessage *flowmessage.FlowMessage, data []byte, config *SFlowMapper) {
	...
	(*flowMessage).SrcMacIn = srcMac
	(*flowMessage).DstMacOut = dstMac
	...
	(*flowMessage).SrcTos = uint32(tos)
	(*flowMessage).MinTTL = uint32(ttl)
...
func SearchSFlowSamplesConfig(samples []interface{}, config *SFlowMapper) []*flowmessage.FlowMessage {
	...
	for _, flowSample := range samples {
		...
		case sflow.FlowSample:
			...
			flowMessage.SamplingRate = uint32(flowSample.SamplingRate)
		...
		for _, record := range records {
			...
			case sflow.SampledHeader:
				flowMessage.BytesIn = uint64(recordData.FrameLength)
			...
			case sflow.SampledIPv4:
				...
				flowMessage.BytesIn = uint64(recordData.Base.Length)
				...
				flowMessage.SrcTos = recordData.Tos
			...
			case sflow.SampledIPv6:
				...
				flowMessage.SrcPort = recordData.Base.SrcPort
				flowMessage.DstPort = recordData.Base.DstPort
				flowMessage.SrcTos = recordData.Priority
			...
			case sflow.ExtendedGateway:
				...
				if len(recordData.ASPath) > 0 {
					...
					//flowMessage.BgpNextHop = recordData.ASPath[0]
